...
{
    return ptDist(p[0], p[1]) + ptDist(p[1], p[2]) + ptDist(p[2], p[3]) + ptDist(p[3], p[0]);
}
...
char classifyComponent(const Component &comp)
{
    ...
    // Прямоугольник
    {
        ...
        if ((double)onEdge / n >= 0.7 && (double)topCnt / w >= 0.5 && (double)botCnt / w >= 0.5 && (double)lCnt / h >= 0.5 && (double)rCnt / h >= 0.5)
            return 'R';
    }
    ...
    // Треугольник
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            if (comp.row[i] < comp.row[ti])
                ti = i;
            if (comp.row[i] * w - comp.col[i] > comp.row[bli] * w - comp.col[bli])
                bli = i;
            if (comp.row[i] * w + comp.col[i] > comp.row[bri] * w + comp.col[bri])
                bri = i;
        }
